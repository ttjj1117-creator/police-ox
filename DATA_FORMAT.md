...
`datasetVersion`은 1 이상의 정수인 정보성 필드입니다. 부분 Dataset의 전역 버전이 기존보다 낮아도 그 숫자만으로 병합을 거부하지 않습니다. 병합 결과에는 두 값 중 큰 값을 유지하며 자동 증가시키지 않습니다. 기본서 목차 설치도 이 값을 증가시키지 않습니다. 실제 콘텐츠 충돌은 아래의 개별 개정 번호로 검사합니다.
